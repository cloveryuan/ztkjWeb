            ...
            this.state.data.map(function (res: any) {
                ...
                if (index) {
                    res.children.map(item => (
                        data1.push({ text: item.text, id: item.id, data: res.id, href: item.href, filename: item.filename })
                    ))
                }
            })
            // console.log(data1)
                        ...
                        <Form className={siderStyle.sidebarForm} style={style}>
                            <div className="inputGroup">
                                ...
                                >
                                    {
                                        Sdata ? (
                                            Sdata.map((d) => <Option key={d.id} ref={d.data} data-filename={d.filename}>{d.text}</Option>)
                                        ) : ''
                                    }
                                </Select>
                            </div>
