    ...
    class extends React.Component <PropsCollectionCreateForm & FormComponentProps, any> {
        ...
        handleSearch = (e) => {
            ...
            this.props.form.validateFields((err, fieldsValue) => {
                ...
                // let uid = values.signallist < 0
                //     ?
                //     -1 : Number(values.signallist.substr(12, 2)) * 10000000 + Number(values.signallist.substr(18, 7))
                let onData = {
                    mtd: 'gwhd',
                    dts: values.dts,
                    dte: values.dte,
                ...
                http.postReq(
                    '/ztmanage/a/zt/searchwarn',
                    onData,
                    (data) => {
                        let searchData = [];
                        if (data.hasOwnProperty('gwhd')) {
                            for (let i = 0; i < data.gwhd.length; i++) {
                                searchData.push(data.gwhd[i]);
                            }
